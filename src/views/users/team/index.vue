<template>
  <div class="app-container">
    <div class="page_head">
      <div class="search_bar clearfix">
        <el-form :model="query" inline label-width="80px">
          <el-form-item>
            <el-input v-model="query.teamName" placeholder="请输入班组名称" prefix-icon="Search" clearable @keyup.enter="handleQuery()" />
          </el-form-item>
          <el-form-item>
      ...
      <div class="button_bar">
        <el-button v-permission="'team:save'" type="primary" @click="openFormModal()">添加班组</el-button>
      </div>
    </div>
    <el-table v-loading="page.loading" :data="page.list" row-key="id">
      <el-table-column label="班组名称" prop="teamName" min-width="120" />
      ...
      <el-table-column :width="210" fixed="right" label="操作">
        <template #default="scope">
          <el-button v-permission="'team:edit'" text type="primary" @click="openFormModal(scope.row)">编辑</el-button>
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item @click="handleToggleStatus(scope.row)">
                  <el-button v-permission="'team:edit'" text type="primary">{{ scope.row.statusId === 1 ? '停用' : '启用' }}</el-button>
                </el-dropdown-item>
                <el-dropdown-item>
                  <el-button v-permission="'team:delete'" text type="primary" @click="handleDeleteTeam(scope.row)">删除</el-button>
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
          </el-dropdown>
    ...
    <team-form ref="formRef" @refresh="handlePage" />
  </div>
</template>
...
<script setup>
  ...
  const { page, handlePage, query, handleQuery, resetQuery, handleDelete } = useTable({
    page: usersApi.teamPage,
    delete: usersApi.teamDelete
  })

  // 删除前把人数带进提示里。后端还会再拦一次，
  // 这里只是让管理员点之前就知道会不会被拒。
  const handleDeleteTeam = (row) => {
    const tip = row.userCount > 0 ? `「${row.teamName}」下还有 ${row.userCount} 名员工，需先调整这些员工的班组才能删除。` : `确认删除班组「${row.teamName}」?`
    handleDelete(row, tip)
  }

  // 不能复用 useTable 的 handleStatus：它只发 { id, statusId }，
  // 而修改接口要求班组名称非空，会被参数校验挡回来。
  const handleToggleStatus = async (row) => {
    const target = row.statusId === 1 ? 0 : 1
    page.loading = true
    try {
      await usersApi.teamEdit({ id: row.id, teamName: row.teamName, remark: row.remark, sort: row.sort, statusId: target })
